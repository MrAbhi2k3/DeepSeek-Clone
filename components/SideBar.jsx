...
const SideBar = ({expand, setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div className={`flex flex-col ${expand ? "gap-8" : "items-center gap-8"}`}>
        <div className={`flex ${expand ? "flex-row justify-between items-center" : "flex-col items-center gap-8"}`}>
          <Image 
      ...
      <div className='mb-4'>
        <div className={`flex items-center group relative ${expand ? "gap-2 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 w-full" : "h-10 w-10 hover:bg-gray-500/30 rounded-lg mx-auto"}`}>
          <Image 
            className={expand ? "w-5" : "w-6 mx-auto"} 
            src={expand ? assets.phone_icon : assets.phone_icon_dull} 
            alt=''
          />
          <div className={`absolute ${expand ? "-top-60 left-0" : "-top-60 -right-40"} pb-8 opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
            <div className='relative w-max bg-black text-sm p-3 rounded-lg shadow-lg'>
